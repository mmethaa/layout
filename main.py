...
# แบ่งกลุ่มเกรด
high = ['MONTARA', 'PRIMAVILLA', 'RIVIERA', 'GRANDVILLE', 'PRIME', 'TASCANY', 'GRANDESSENCE', 'ESSENCE']
mid = ['PALMSPRING', 'VILLE', 'PARKVILLE', 'GARDENVILLE', 'PALMVILLE']
low = ['BELLA', 'PRIMO', 'WATTANALAI', 'PRIDE']

def classify_grade(g):
    if g in high: return 'High'
    elif g in mid: return 'Mid'
    elif g in low: return 'Low'
    return 'Unknown'

df['กลุ่มเกรด'] = df['เกรดโครงการ'].map(classify_grade)
avg_ซอยต่อกลุ่ม = df.groupby('กลุ่มเกรด')['หลังต่อซอย'].mean().to_dict()
...
if submitted:
    ...
    if group == 'High':
        ...
        คำเตือน = "🏚️ เน้นทาวน์โฮม/บ้านแฝด"

    ซอย = หลังรวม / avg_ซอยต่อกลุ่ม.get(group, 12)
